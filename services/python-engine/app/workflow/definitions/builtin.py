...
def build_builtin_node_definitions(runtime_profile: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "inputs": [
                _port("token_corpus_in", "FilteredTokenCorpus", "分析词项"),
                _port("term_table_in", "AnyTable", "候选词表"),
            ],
            "outputs": [
                _port("focused_token_corpus", "FilteredTokenCorpus", "聚焦后词项"),
                _port(
                    "focus_term_summary",
                    "AnyTable",
                    "聚焦摘要",
                    result_bundle_key="focus_term_summary",
                    include_in_html_audit=True,
                ),
            ],
            "params": [
                _enum_param(
                    ...
                    "词项来源",
                    "auto",
                    [("auto", "自动识别"), ("feature_terms", "特征词表"), ("keywords", "关键词表")],
                ),
                _string_param("term_field", "词项字段", "auto"),
                _number_param("max_terms", "最大词项数", 80),
                _bool_param("selected_only", "仅保留已选特征词", True),
                _bool_param("project_keywords_only", "仅项目级关键词", True),
                _enum_param(
                    "on_empty",
                    "无候选词时",
                    "pass_through",
                    [("pass_through", "沿用上游"), ("empty", "输出空词项")],
                ),
            ],
            "runtime": _runtime("filtering", "workflow.focus_terms", cacheable=True, previewable=True),
        },
        _analysis_node(
